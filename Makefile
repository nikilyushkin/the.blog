...
docker-run-production:  ## Runs production server in docker
	python3 manage.py migrate
	gunicorn heynik_blog.asgi:application -w 5 -k uvicorn.workers.UvicornWorker --bind=0.0.0.0:8022 --capture-output --log-level debug --access-logfile - --error-logfile -
...
migrate:  ## Migrate database to the latest version (inside container)
	python3 manage.py migrate
